...
This project is **API + frontend only** — a multi-user React SPA + Fastify API + SQLite, where users register, answer themed *studies*, swipe a generated "does this sound like you?" card deck, and generate a per-user `soul.md`. **SQLite (`data/soul.sqlite`) is the source of truth.** (There is **no** conversation/WhatsApp import *into the soul pipeline* and **no** eval harness — both were removed; the only extraction input source is the questionnaire.)

There is also a **standalone, no-LLM conversation-statistics feature** (Analyze + Saved pages), independent of the `soul.md` pipeline: a pasted chat export is parsed + reduced to aggregate numbers on the server **transiently — the transcript is never stored** — and only the derived statistics can be saved. See "Conversation statistics" below.
...
- **`src/stats/`** — `conversation-stats.ts`: the standalone, no-LLM conversation analyzer (WhatsApp/Telegram-style parser + pure aggregation). Used only by the stats routes; unrelated to the soul pipeline.
- **`src/server/`** — `app.ts` builds Fastify (cookie plugin, 5 route groups, prod static SPA + history fallback), `serve.ts` listens, `auth.ts` (scrypt hashing + opaque session tokens in a signed `sid` cookie + `requireAuth` preHandler), `pipeline.ts` (per-user extraction), `swipe.ts` (card generation), `ollama-ready.ts` (shared TTL-cached readiness ping), `routes/{auth,studies,swipe,stats,results}.ts`.
- **`frontend/`** — own Vite root + own `tsconfig.json` (DOM lib + `react-jsx` + Bundler resolution — kept separate from the backend's NodeNext tsconfig). Base UI (`@base-ui-components/react`), `react-router-dom`. Pages: Login/Register, Intro (`/`), Studies index + StudyPage, Answers, Cards (`SwipePage` — the swipe deck), Analyze (`StatsPage`) + Saved (`SavedStatsPage` + detail), Results (renders `soul.md`, runs extraction). `vite.config.ts` proxies `/api`→4317 in dev; `vite build`→`dist/public` for prod.
...
A **transient, no-LLM** feature, fully separate from the soul pipeline. `POST /api/stats/compute` takes a pasted chat export (`{ conversation }`), runs `analyzeConversation` (parse → per-participant message/word counts, avg response time, top words, messages-per-month, behavioural red flags), and returns the `ConversationStats` — **the transcript is never written to disk or DB**. `POST /api/stats/save` persists only the derived JSON to `saved_stats` under a user-given name (default `<index>-<YYYY-MM-DD>`, computed server-side; `UNIQUE(user_id, name)` → 409 on clash). `GET /api/stats` (list), `GET /api/stats/:id` (detail), `DELETE /api/stats/:id`. The parser handles WhatsApp/Telegram Android-dash + iOS-bracket formats with file-wide day/month-order resolution; golden tests live in `src/stats/conversation-stats.test.ts`. The UI is `StatsPage` (paste/upload → Analyze → name → Save) + `SavedStatsPage`/detail.

### Privacy boundary
Everything under the per-user `WORK_DIR` scratch (`processed/`, `chunks/`), `.cache/`, `out/`, plus the web platform's `data/` (SQLite) is gitignored. `soul.md` is meant for manual review before downstream use — the safety net against verbatim regurgitation by the LLM. The conversation-statistics feature is privacy-first by construction: the pasted transcript is analyzed in-memory and discarded — only aggregate numbers are ever persisted (`saved_stats`). UI-audit screenshots live under `playwright/screenshots/` (gitignored).
